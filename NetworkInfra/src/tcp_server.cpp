...
TCPServer::TCPServer(EventLoop* loop, const SocketAddress& listen_addr, const std::string& name)
    : loop_(loop),
      ...
      next_conn_id_(0)
{
    acceptor_.set_new_connection_handler(std::bind(&TCPServer::HandleNewConnection, this, _1, _2));
}
...
void TCPServer::HandleNewConnection(ScopedFD&& conn_fd, const SocketAddress& peer_addr)
{
    ...
    auto conn = std::make_shared<TCPConnection>(loop_, conn_name, std::move(conn_fd),
