...
class ProtobufConan(ConanFile):
    ...
    url = "https://gitlab.zoom.us/vendors/protobuf"
    ...
    def requirements(self):
        ...
        if self._protobuf_release >= "30.1":
            self.requires("abseil-cpp/[>=20230802.1 <=20260107.1]", transitive_headers=True, transitive_libs=True)
            ...
            self.requires("abseil-cpp/[>=20230802.1 <=20250127.0]", transitive_headers=True, transitive_libs=True)
    ...
    def validate(self):
        ...
        if is_msvc(self) and self._protobuf_release >= "22" and self.options.shared and \
            not self.dependencies["abseil-cpp"].options.shared:
        ...
        # if "abseil-cpp" in self.dependencies.host:
        abseil_cppstd = self.dependencies.host.get('abseil-cpp').info.settings.compiler.cppstd
    ...
    def package_info(self):
        ...
        if self._protobuf_release >= "22.0":
            ...
            self_ver = str.format(f"{ver.major}.{ver.minor}.{ver.patch}")
            absl_deps = [f"abseil-cpp::{c}" for c in self.conan_data["absl_deps"][self_ver]]
        ...
        if self._protobuf_release >= "22.0" and (not self.options.shared or self.options.get_safe("upb")):
            ...
            self.cpp_info.components["utf8_validity"].requires = ["abseil-cpp::absl_strings"]
