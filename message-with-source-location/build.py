...
class uniconf_subsystem():
    ...
    def generate(self):
        ...
        cmd.append(f'-DMESSAGESOURCELOCATION_ENABLE_CLANG_TIDY={clang_tidy}')

        sanitizer = 'ON' if self._sanitizer else 'OFF'
        cmd.append(f'-DMESSAGESOURCELOCATION_USE_SANITIZER={sanitizer}')
...
class multiconf_subsystem():
    ...
    def generate(self):
        ...
        if self._clang_tidy:
            cmd.append('-DMESSAGESOURCELOCATION_ENABLE_CLANG_TIDY=ON')
