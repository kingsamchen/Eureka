...
void async_getline(IoExecutor io_ex, std::istream& is, Handler handler) {
    ...
    boost::asio::require(io_ex, execution::blocking.never).execute([&is, work_ex, handler = std::move(handler)]() mutable {
        std::string line;
        std::getline(is, line);

        // Pass the result to the handler, via the associated executor.
        boost::asio::prefer(work_ex, execution::blocking.possibly).execute([line = std::move(line), handler = std::move(handler)]() mutable {
...
void async_getlines(IoExecutor io_ex, std::istream& is, std::string init, Handler handler) {
    ...
    async_getline(io_work_ex, is,
                  bind_executor(handler_work_ex,
                                [io_work_ex, &is, lines = std::move(init), handler = std::move(handler)](std::string line) mutable {
                                    if (line.empty())
                                        handler(lines);
                                    else
