...
std::future<void> pipeline(queue_back<T> in, F f, Tail... t) {
    ...
    // Run the function.
    boost::asio::require(ex, execution::blocking.never).execute([in, out, f = std::move(f)]() mutable {
        f(in, out);
        out.stop();
    });
...
std::future<void> pipeline(F f, Tail... t) {
    ...
    // Run the function.
    boost::asio::require(ex, execution::blocking.never).execute([out, f = std::move(f)]() mutable {
        f(out);
        out.stop();
    });
