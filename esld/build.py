...
class uniconf_subsystem():
    def __init__(self, params):
        ...
        dir_name = self._gen.strip().replace(' ', '-').lower() +\
                   '-' + self._build_type
        self._out = pathlib.Path(self._src) / 'build' / dir_name
    ...
    def generate(self):
        ...
        cmd.append(f'-DESLD_ENABLE_CLANG_TIDY={clang_tidy}')

        sanitizer = 'ON' if self._sanitizer else 'OFF'
        cmd.append(f'-DESLD_USE_SANITIZER={sanitizer}')
...
class multiconf_subsystem():
    def __init__(self, params):
        ...
        if normed_gen == '' and platform.system() == OS_WIN:
            normed_gen = 'msvc'
        self._out = pathlib.Path(self._src) / 'build' / normed_gen
