    ...
    case RequestParseStage::Preamble: {
        ...
        asio::async_read(
                ...
                [self, p = std::move(packet), this](std::error_code ec, size_t bytes) mutable {
                    ...
                    assert(bytes == RequestPacket::kPreambleSize);
                    p->dest_port = esl::byteswap(p->dest_port);
                    p->dest_ip = esl::byteswap(p->dest_ip);
    ...
    case RequestParseStage::UserId: {
        ...
        asio::async_read_until(
                ...
                [self, p = std::move(packet), this](std::error_code ec, size_t bytes) mutable {
                    ...
                    auto next_stage = p->dest_ip < 256 ? RequestParseStage::Domain : RequestParseStage::Complete;
...
void Tunnel::ConnectRemote(std::unique_ptr<RequestPacket> packet) {
    ...
    asio::async_connect(
            ...
            [this, self = shared_from_this()](std::error_code ec, const tcp::endpoint& endpoint) {
                ...
                auto port_in_be = esl::byteswap(endpoint.port());
