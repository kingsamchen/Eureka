        ...
        case RequestParseStage::DestPort: {
            ...
            asio::async_read(
                ...
                [this, self=shared_from_this(), pkt=std::move(packet)](auto ec, auto) mutable {
                    ...
                    pkt->dest_port = kbase::NetworkToHost(pkt->dest_port);
...
void Tunnel::ConnetRemote(std::unique_ptr<RequestPacket> packet)
{
    ...
    if (packet->addr_type == kAddrIPv4) {
        ENSURE(CHECK, packet->dest_addr.size() == 4)(packet->dest_addr.size()).Require();
        auto ip = kbase::NetworkToHost(*reinterpret_cast<const uint32_t*>(packet->dest_addr.data()));
        endpoints.emplace_back(asio::ip::make_address_v4(ip), packet->dest_port);
    } else if (packet->addr_type == kAddrIPv6) {
        ENSURE(CHECK, packet->dest_addr.size() == 16)(packet->dest_addr.size()).Require();
...
void Tunnel::ReplyRequest()
{
    ...
    auto port_in_be = kbase::HostToNetwork(client_sock_.local_endpoint().port());
    ...
    asio::async_write(
        ...
        [this, self=shared_from_this(), reply](auto ec, auto bytes) {
            RETAIN_LIFETIME_TO_HERE(reply);
            ENSURE(CHECK, bytes==10)(bytes).Require();
            if (ec) {
